...
\item We aim to study the effect of the economy on mortality, not vice versa. Before the COVID-19 pandemic, the effect of mortality fluctuations on the economy was considered negligible. We study data up to 2018.
...
\item An R implementation of the Hodrick-Prescott filter is \code{hpfilter} in the R package \code{mFilter}. This can be installed by

<<mFilter,eval=FALSE,echo=TRUE>>=
...
\bi
\item There is strong evidence of pro-cyclical mortality at a national level in the USA from 1948 to 2018. For example, the Great Recession of 2009-2010 led to high unemployment, but these two years had above-trend values of life expectancy at birth.
...
\item The difference is small compared to the standard error. Overall there is consistency through this 71 year interval, though the pattern is weak in the 1990s.
