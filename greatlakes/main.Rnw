...
\begin{itemize}
\item
The \texttt{greatlakes} subdirectory of the \texttt{531w24} git repository has a file \texttt{test.sbat} which submits a batch job running the parallel \texttt{foreach} test in \texttt{test.R}.
\item
